...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:peppermint_tea) { described_class.new('peppermint tea') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(peppermint_tea.type).to eq('peppermint tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(peppermint_tea.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:peppermint_tea) { described_class.new('peppermint tea', 20) }

      # remove the 'x' before running this test
      it 'is full' do
        expect(peppermint_tea).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:peppermint_tea) { described_class.new('peppermint tea', 8) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(peppermint_tea).not_to be_full
      end
    end
  end
end
